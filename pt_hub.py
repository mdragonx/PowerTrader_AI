...
class PowerTraderHub(tk.Tk):
    ...
    def open_settings_dialog(self) -> None:
        ...
        # --- Bitso API setup (writes bitso_key.txt + bitso_secret.txt used by pt_trader.py) ---
        def _api_paths() -> Tuple[str, str]:
            key_path = os.path.join(self.project_dir, "bitso_key.txt")
            secret_path = os.path.join(self.project_dir, "bitso_secret.txt")
        ...
        def _refresh_api_status() -> None:
            ...
            missing = []
            if not k:
                missing.append("bitso_key.txt (API Key)")
            if not s:
                missing.append("bitso_secret.txt (API Secret)")
        ...
        def _open_api_folder() -> None:
            """Open the folder where bitso_key.txt / bitso_secret.txt live."""
            try:
        ...
        def _clear_api_files() -> None:
            """Delete bitso_key.txt / bitso_secret.txt (with a big confirmation)."""
            key_path, secret_path = _api_paths()
            ...
            _refresh_api_status()
            messagebox.showinfo("Deleted", "Deleted bitso_key.txt and bitso_secret.txt.")

        def _open_bitso_api_wizard() -> None:
            """
            Beginner-friendly wizard that stores Bitso API credentials.

            What we store:
              - bitso_key.txt    = your Bitso *API Key*
              - bitso_secret.txt = your Bitso *API Secret* (treat like a password)
            """
            import time
            import hmac
            import hashlib
            ...
            wiz = tk.Toplevel(win)
            wiz.title("Bitso API Setup")
            wiz.geometry("820x520")
            wiz.minsize(720, 460)
            wiz.configure(bg=DARK_BG)

            key_path, secret_path = _api_paths()
            existing_api_key, existing_api_secret = _read_api_files()

            intro = (
                "This trader uses Bitso's API keys.\n\n"
                "✅ Steps:\n"
                "  1) Create API keys in your Bitso account (API keys page).\n"
                "  2) Paste the API Key and API Secret below.\n"
                "  3) Click Save.\n\n"
                "This wizard will save two files next to pt_hub.py:\n"
                "  - bitso_key.txt    (API Key)\n"
                "  - bitso_secret.txt (API Secret)\n"
            )
            ttk.Label(wiz, text=intro, justify="left").pack(anchor="w", padx=16, pady=(16, 8))

            form = ttk.Frame(wiz)
            form.pack(fill="x", padx=16)
            form.columnconfigure(1, weight=1)

            ttk.Label(form, text="API Key:").grid(row=0, column=0, sticky="w", pady=6)
            api_key_var = tk.StringVar(value=existing_api_key or "")
            ttk.Entry(form, textvariable=api_key_var).grid(row=0, column=1, sticky="ew", pady=6)

            ttk.Label(form, text="API Secret:").grid(row=1, column=0, sticky="w", pady=6)
            api_secret_var = tk.StringVar(value=existing_api_secret or "")
            ttk.Entry(form, textvariable=api_secret_var, show="•").grid(row=1, column=1, sticky="ew", pady=6)
            ...
            def _test_credentials() -> None:
                ...
                api_secret = (api_secret_var.get() or "").strip()
                ...
                if not api_key or not api_secret:
                    messagebox.showerror("Missing credentials", "Paste both API Key and API Secret.")
                    return

                path = "/v3/balance/"
                nonce = int(time.time() * 1000)
                msg = f"{nonce}GET{path}"
                signature = hmac.new(
                ...
                try:
                    resp = requests.get(f"https://api.bitso.com{path}", headers=headers, timeout=10)
                    if resp.status_code >= 400:
                        messagebox.showerror("Test failed", f"Bitso returned HTTP {resp.status_code}.\n\n{resp.text}")
                        return
                    data = resp.json()
                    if not data.get("success"):
                        messagebox.showerror("Test failed", f"Bitso error:\n\n{data}")
                        return
                    messagebox.showinfo("Test successful", "✅ Bitso accepted your API credentials.")
                except Exception as e:
                    messagebox.showerror("Test failed", f"Couldn't reach Bitso.\n\nError:\n{e}")
            ...
            def do_save():
                ...
                messagebox.showinfo(
                    ...
                    f"  API Secret → {os.path.abspath(secret_path)}\n"
                )
                wiz.destroy()

            ttk.Button(btns, text="Test Credentials (safe, no trading)", command=_test_credentials).pack(side="left")
            ttk.Button(btns, text="Save", command=do_save).pack(side="left", padx=8)
            ttk.Button(btns, text="Close", command=wiz.destroy).pack(side="left")

        ttk.Label(frm, text="Bitso API:").grid(row=r, column=0, sticky="w", padx=(0, 10), pady=6)
        ...
        ttk.Button(api_row, text="Setup Wizard", command=_open_bitso_api_wizard).grid(row=0, column=1, sticky="e", padx=(10, 0))
